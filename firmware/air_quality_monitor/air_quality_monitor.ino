...
void generateMQ135Simulated(float temperature, float humidity, float &co2, float &no2, float &nh3) {
...
// ==================== READ MQ135 SENSOR ====================
void readMQ135(int raw, float temperature, float humidity, float &co2, float &no2, float &nh3) {
  #if !ENABLE_MQ135
...
void publishSensorData() {
  ...
  if (mqttClient.publish(MQTT_TOPIC, payload, true)) {
    Serial.printf("📤 Published: T=%.1f°C H=%d%% CO2=%dppm MQ2=%dppm Dust=%.1fµg/m³ MQ135=%s\n",
                  temperature, (int)humidity, (int)co2, (int)mq2Ppm, dustDensity,
                  mq135UsingSimulation ? "sim" : "real");
  } else {
